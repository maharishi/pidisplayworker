...
namespace pidisplayworker
{
    ...
    public class DHT11 : IDHT11
    {
        ...
        public DHT11(IGpioPin datatPin, SensorTypes sensor)
        {
            if (datatPin != null)
            {
                _dataPin = datatPin;
        ...
        private async Task<UInt32> ExpectPulse(GpioPinValue level)
        {
            return await Task.Run<UInt32>(() =>
            {
                ...
                long end = DateTime.Now.AddMilliseconds(0.1).Ticks;
                
                Predicate<GpioPinValue> CheckDataValue = level => {
                    var val = _dataPin.Read() ? GpioPinValue.High : GpioPinValue.Low;
                    return val == level; 
                };

                while (CheckDataValue(level))
                {
                    count++;
        ...
        private void DelayMicroseconds(int secs)
        {
